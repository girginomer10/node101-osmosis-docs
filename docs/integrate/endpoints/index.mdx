...
- [node101](https://node101.io/en/rpc/osmosis) provides paid Osmosis RPC and managed dedicated nodes.
  - JSON-RPC, REST, and gRPC access; distributed request handling, managed monitoring, and 24/7 support.
  - HTTP/WebSocket access, the 101 Dashboard, and incident reports on Business, Expert, and Enterprise plans.
  - Archive nodes, testnet RPC, and deployments in Türkiye, Europe, and the United States available on request.
  - Paid access is arranged during onboarding; no free public RPC endpoint is offered. [Service-Level Agreement](https://node101.io/legal/en/service-level-agreement.pdf).
